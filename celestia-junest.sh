...
DEPENDENCES="gtkglext" #SYNTAX: "APP1 APP2 APP3 APP4...", LEAVE BLANK IF NO OTHER DEPENDENCIES ARE NEEDED
...
LIBSAVED="libSM.so libICE.so libuuid.so pixmap libGLX libsensors.so libpciaccess.so libtheora $lib_audio_keywords $lib_browser_launcher"
...
if ! test -d ./Celestia-appimage; then
	...
	rsync -av Celestia-appimage/textures/hires/* ./"$APP".AppDir/.junest/usr/share/celestia/textures/hires/
	cd ./"$APP".AppDir/.junest/usr/share/celestia/textures/
	rm -Rf ./medres ./lores
	ln -s hires lores
	ln -s hires medres
	cd -
	rm -Rf ./Celestia-appimage
fi

# Fix libtheora.so.0
cp -r ./"$APP".AppDir/.junest/usr/lib/libtheora.so ./"$APP".AppDir/.junest/usr/lib/libtheora.so.0
